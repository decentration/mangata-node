...
use frame_support::{
	dispatch::{DispatchResultWithPostInfo, DispatchErrorWithPostInfo}};
use crate::messages::L1UpdateRequest;
use sp_core::{H256};
...
mod benchmarks {
	...
	where T::AccountId: From<[u8;20]> 
		{
		T::Tokens::mint(TOKEN_ID.into(), &who, MINT_AMOUNT.to_balance::<T>())?;
		let (l1_aset_chain, l1_asset_address) = match T::AssetRegistryProvider::get_asset_l1_id(TOKEN_ID.into()).ok_or(Error::<T>::L1AssetNotFound)?{
	...
	fn setup_sequencer<T:Config>(sequencer: T::AccountId, sequencer_alias: Option<T::AccountId>, should_be_active: bool, should_be_selected: bool) -> DispatchResultWithPostInfo
	where <T as pallet_sequencer_staking::Config>::ChainId: From<messages::Chain>,
			T::AccountId: From<[u8;20]> 
	{
		...
		if let Some(ref sequencer_alias) = sequencer_alias{
			...
			T::Tokens::mint(T::NativeCurrencyId::get(), &sequencer_alias, MINT_AMOUNT.to_balance::<T>())?;
		}
		...
		if should_be_selected{
			pallet_sequencer_staking::SelectedSequencer::<T>::mutate(|selected| selected.insert(l1_aset_chain.into(), sequencer.into()));
		}

		Ok(().into())
	}

	fn get_chain_and_address_for_asset_id<T:Config>(asset_id: CurrencyIdOf<T>) -> Result<(crate::messages::Chain, [u8; 20]), DispatchErrorWithPostInfo>{
		let (l1_aset_chain, l1_asset_address) = match T::AssetRegistryProvider::get_asset_l1_id(TOKEN_ID.into()).ok_or(Error::<T>::L1AssetNotFound)?{
			L1Asset::Ethereum(v) => {(crate::messages::Chain::Ethereum, v)},
			L1Asset::Arbitrum(v) => {(crate::messages::Chain::Arbitrum, v)},
		};
	...
	fn create_batch() -> Result<(), BenchmarkError> {
		...
		setup_sequencer::<T>(SEQUENCER_ACCOUNT.into(), Some(SEQUENCER_ALIAS_ACCOUNT.into()), false, false)?;

		ManualBatchExtraFee::<T>::set(MANUAL_BATCH_EXTRA_FEE.to_balance::<T>());

		let (l1_aset_chain, l1_asset_address) = get_chain_and_address_for_asset_id::<T>(TOKEN_ID.into())?;
		assert!(L2RequestsBatch::<T>::get::<(<T as Config>::ChainId,_)>((l1_aset_chain.into(), FIRST_BATCH_ID)).is_none(), "BEFORE L2RequestsBatch {:?} chain {:?} batch should be uninit", l1_aset_chain, FIRST_BATCH_ID);
		assert!(L2RequestsBatchLast::<T>::get().get(&l1_aset_chain.into()).is_none(), "BEFORE L2RequestsBatchLast {:?} chain should be uninit", l1_aset_chain);

		let acc: T::AccountId = SEQUENCER_ALIAS_ACCOUNT.into();
		whitelist_account!(acc);
		#[extrinsic_call]
		_(RawOrigin::Signed(SEQUENCER_ALIAS_ACCOUNT.into()), l1_aset_chain.into(), Some(SEQUENCER_ACCOUNT.into()));
	
		assert!(L2RequestsBatch::<T>::get::<(<T as Config>::ChainId,_)>((l1_aset_chain.into(), FIRST_BATCH_ID)).is_some(), "AFTER L2RequestsBatch {:?} chain {:?} batch should be init", l1_aset_chain, FIRST_BATCH_ID);
		assert!(L2RequestsBatchLast::<T>::get().get(&l1_aset_chain.into()).is_some(), "BEFORE L2RequestsBatchLast {:?} chain should be init", l1_aset_chain);
	...
	fn update_l2_from_l1(x: Linear<2, 200> ) -> Result<(), BenchmarkError> {
		...
		setup_sequencer::<T>(SEQUENCER_ACCOUNT.into(), Some(SEQUENCER_ALIAS_ACCOUNT.into()), true, true)?;
		let (l1_aset_chain, l1_asset_address) = get_chain_and_address_for_asset_id::<T>(TOKEN_ID.into())?;
		...
		let dispute_period_end = <frame_system::Pallet<T>>::block_number().saturated_into::<u128>() + Rolldown::<T>::get_dispute_period();
		assert!(PendingSequencerUpdates::<T>::get(dispute_period_end, l1_chain).is_none(), "BEFORE PendingSequencerUpdates {:?} dispute_period_end {:?} l1_chain should be uninit", dispute_period_end, l1_chain);
		...
		assert!(PendingSequencerUpdates::<T>::get(dispute_period_end, l1_chain).is_some(), "AFTER PendingSequencerUpdates {:?} dispute_period_end {:?} l1_chain should be init", dispute_period_end, l1_chain);
		Ok(())
	}
	...
	fn force_update_l2_from_l1(x: Linear<2, 200> ) -> Result<(), BenchmarkError> {
		...
		assert!(MaxAcceptedRequestIdOnl2::<T>::get(l1_chain).is_zero(), "BEFORE MaxAcceptedRequestIdOnl2 {:?} l1_chain should be zero", l1_chain);

		#[extrinsic_call]
		_(RawOrigin::Root, update);
		
		assert!(!MaxAcceptedRequestIdOnl2::<T>::get(l1_chain).is_zero(), "AFTER MaxAcceptedRequestIdOnl2 {:?} l1_chain should NOT be zero", l1_chain);
		Ok(())
	}
	...
	fn cancel_requests_from_l1() -> Result<(), BenchmarkError> {
		...
		let update = L1UpdateBuilder::default().with_requests(
				vec![L1UpdateRequest::Deposit(Default::default())])
			.build();
		
		let sequencer_account: T::AccountId = SEQUENCER_ACCOUNT.into();
		PendingSequencerUpdates::<T>::insert(DUMMY_REQUEST_ID, l1_chain, (sequencer_account, update, H256::default()));

		assert!(L2Requests::<T>::get(l1_chain, RequestId::from((Origin::L2, FIRST_REQUEST_ID))).is_none(), "BEFORE L2Requests {:?} l1_chain {:?} requestId should be none", l1_chain, FIRST_REQUEST_ID);
		...
		assert!(L2Requests::<T>::get(l1_chain, RequestId::from((Origin::L2, FIRST_REQUEST_ID))).is_some(), "AFTER L2Requests {:?} l1_chain {:?} requestId should be some", l1_chain, FIRST_REQUEST_ID);
		Ok(())
	}
	...
	fn force_cancel_requests_from_l1() -> Result<(), BenchmarkError> {
		...
		let sequencer_account: T::AccountId = SEQUENCER_ACCOUNT.into();
		PendingSequencerUpdates::<T>::insert(DUMMY_REQUEST_ID, l1_chain, (sequencer_account, messages::L1Update::default(), H256::default()));

		assert!(T::SequencerStakingProvider::is_selected_sequencer(l1_chain, &SEQUENCER_ACCOUNT.into()), "suboptimal code path");
		assert!(PendingSequencerUpdates::<T>::get(DUMMY_REQUEST_ID, l1_chain).is_some(), "BEFORE PendingSequencerUpdates {:?} requestId {:?} l1_chain should be some", DUMMY_REQUEST_ID, l1_chain);

		#[extrinsic_call]
		_(RawOrigin::Root, l1_aset_chain.into(), DUMMY_REQUEST_ID);

		assert!(PendingSequencerUpdates::<T>::get(DUMMY_REQUEST_ID, l1_chain).is_none(), "AFTER PendingSequencerUpdates {:?} requestId {:?} l1_chain should be none", DUMMY_REQUEST_ID, l1_chain);
		Ok(())
	}
	...
	fn withdraw() -> Result<(), BenchmarkError>{
		...
		assert!(!<<T as Config>::WithdrawFee>::convert(l1_chain).is_zero(), "withdraw fee should not be zero, suboptimal code path");
		assert!(L2Requests::<T>::get(l1_chain, RequestId::from((Origin::L2, FIRST_REQUEST_ID))).is_none(), "BEFORE L2Requests {:?} l1_chain {:?} requestId should be none", l1_chain, DUMMY_REQUEST_ID);
		...
		assert!(L2Requests::<T>::get(l1_chain, RequestId::from((Origin::L2, FIRST_REQUEST_ID))).is_some(), "AFTER L2Requests {:?} l1_chain {:?} requestId should be some", l1_chain, DUMMY_REQUEST_ID);
		Ok(())
	...
	fn refund_failed_deposit() -> Result<(), BenchmarkError>{
		...
		FailedL1Deposits::<T>::insert((l1_chain, DUMMY_REQUEST_ID), (<T::AccountId as From<[u8;20]>>::from(USER_ACCOUNT), H256::default()));
	...
	fn ferry_deposit() -> Result<(), BenchmarkError>{
		...
		assert!(FerriedDeposits::<T>::get((l1_chain, deposit_hash)).is_none(), "BEFORE FerriedDeposits {:?} l1_chain {:?} deposit_hash should be none", l1_chain, deposit_hash);
		...
		assert!(FerriedDeposits::<T>::get((l1_chain, deposit_hash)).is_some(), "AFTER FerriedDeposits {:?} l1_chain {:?} deposit_hash should be some", l1_chain, deposit_hash);
		Ok(())
	}
	...
	fn process_deposit() -> Result<(), BenchmarkError>{
		...
		FerriedDeposits::<T>::insert((l1_chain, deposit_hash), &<T::AccountId as From<[u8;20]>>::from(ETH_RECIPIENT_ACCOUNT));

		let balance_before = T::Tokens::free_balance(TOKEN_ID.into(), &ETH_RECIPIENT_ACCOUNT.into());

		#[block]
		{Rolldown::<T>::process_deposit(
			l1_chain,
			&deposit
		);}

		let balance_after = T::Tokens::free_balance(TOKEN_ID.into(), &ETH_RECIPIENT_ACCOUNT.into());
		assert_eq!(balance_after, balance_before + DEPOSIT_AMOUNT.into(), "Balance should increase by {:?}", DEPOSIT_AMOUNT);
		Ok(())
	}
	...
	fn process_cancel_resolution() -> Result<(), BenchmarkError>{
		...
		pallet_sequencer_staking::SequencerStake::<T>::insert((<T::AccountId as From<[u8;20]>>::from(SEQUENCER_ACCOUNT), Into::<<T as pallet_sequencer_staking::Config>::ChainId>::into(l1_aset_chain)), slash_fine_amount);
		assert!(pallet_sequencer_staking::SequencerStake::<T>::get((<T::AccountId as From<[u8;20]>>::from(SEQUENCER_ACCOUNT), Into::<<T as pallet_sequencer_staking::Config>::ChainId>::into(l1_aset_chain))) < pallet_sequencer_staking::MinimalStakeAmount::<T>::get() + slash_fine_amount, "suboptimal code path");

		assert!(T::SequencerStakingProvider::is_active_sequencer(l1_chain, &SEQUENCER_ACCOUNT.into()), "suboptimal code path");
		assert!(T::SequencerStakingProvider::is_active_sequencer(l1_chain, &SEQUENCER_ACCOUNT_2.into()), "suboptimal code path");
		assert!(T::SequencerStakingProvider::is_selected_sequencer(l1_chain, &SEQUENCER_ACCOUNT.into()), "suboptimal code path");

		assert!(AwaitingCancelResolution::<T>::get(l1_chain).get(&(SEQUENCER_ACCOUNT.into(), DUMMY_REQUEST_ID, DisputeRole::Submitter)).is_some(), "BEFORE AwaitingCancelResolution {:?} l1_chain {:?} sequencer should be some", l1_chain, SEQUENCER_ACCOUNT);
		assert!(AwaitingCancelResolution::<T>::get(l1_chain).get(&(SEQUENCER_ACCOUNT_2.into(), DUMMY_REQUEST_ID, DisputeRole::Canceler)).is_some(), "BEFORE AwaitingCancelResolution {:?} l1_chain {:?} sequencer should be some", l1_chain, SEQUENCER_ACCOUNT_2);

		let balance_before = T::Tokens::total_balance(T::NativeCurrencyId::get(), &SEQUENCER_ACCOUNT.into());

		#[block]
		{Rolldown::<T>::process_cancel_resolution(
			l1_chain,
			&cancel_resolution
		);}

		let balance_after = T::Tokens::total_balance(T::NativeCurrencyId::get(), &SEQUENCER_ACCOUNT.into());
		assert_eq!(balance_after + TryInto::<u128>::try_into(slash_fine_amount).ok().expect("should work").to_balance::<T>(), balance_before, "Balance should decrease by {:?}", slash_fine_amount);
		assert!(AwaitingCancelResolution::<T>::get(l1_chain).get(&(SEQUENCER_ACCOUNT.into(), DUMMY_REQUEST_ID, DisputeRole::Submitter)).is_none(), "AFTER AwaitingCancelResolution {:?} l1_chain {:?} sequencer should be none", l1_chain, SEQUENCER_ACCOUNT);
		assert!(AwaitingCancelResolution::<T>::get(l1_chain).get(&(SEQUENCER_ACCOUNT_2.into(), DUMMY_REQUEST_ID, DisputeRole::Canceler)).is_none(), "AFTER AwaitingCancelResolution {:?} l1_chain {:?} sequencer should be none", l1_chain, SEQUENCER_ACCOUNT_2);
